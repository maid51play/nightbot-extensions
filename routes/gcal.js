...
router.get('/:id', async function(req, res, next) {
  ...
  const events = calendarJson.items.map(event => `${moment(event.start.dateTime).tz(calendarJson.timeZone).format(dateFormat).replace("am","a").replace("pm","p").replace(":00","")}: ${event.summary}`.split(" ").join(String.fromCharCode(160))).join(" ")
